...
for (i in 1:nrow(md)) {
  if (!is.na(md$Latitude[i])) {
    alcada_srtm3[[i]] <- GNsrtm3(md$Latitude[i], md$Longitude[i])
    alcada_gtopo30[[i]] <- GNgtopo30(md$Latitude[i], md$Longitude[i])
  } else {
    alcada_srtm3[[i]] <- c("-", NA, NA)
    alcada_gtopo30[[i]] <- c("-", NA, NA)
  }
}
...
md <- md %>% 
  ...
  mutate(alcada_gtopo30=alcada_gtopo30.df$gtopo30)
...
for (i in 1:nrow(md)) {
  if (!is.na(md$Latitude[i])) {
      data[[i]] <- xmlToList(xmlParse(paste0("http://api.geonames.org/findNearby?lat=", 
                                         md$Latitude[i], "&lng=",
                                         md$Longitude[i], "&username=xavidp")))
      if (grep("has been exceeded", data[[589]]$status, fixed=T)) {
        nom_lloc[[i]] <- c(rep("-",4))
        cat("Hourly limit exceeded\n")
      } else {
          nom_lloc[[i]] <- c(md$Latitude[i],
                             md$Longitude[i],
                             data[[i]]$geoname$toponymName,
                             data[[i]]$geoname$name)
      }
...
# convert list of results into data.frame
nom_lloc.df <- do.call(rbind.data.frame, nom_lloc)
colnames(nom_lloc.df) <- c("Lat", "Lon", "Comarca", "Lloc")
md <- md %>% 
  mutate(nom_comarca=nom_lloc.df[3],
         nom_lloc=nom_lloc.df[4])
...
```{r}
# See https://cran.r-project.org/web/packages/rwunderground/index.html
```
