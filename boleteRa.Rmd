...
if (FALSE){
md <- rbind(md.orig, md.new)
md <- md %>%
  arrange(cid)

  fname.md.new.edited <- paste0("_120000_", format(Sys.Date(), "%y%m"), "00_md.new.edited.csv")
  md.new.edited <- readr::read_csv(file.path(path.data, fname.md.new.edited))
}
...
if (FALSE) {
  ...
  for (i in 1:nrow(md)) {
    if (!is.na(md$Latitude[i])) {
      #      data[[i]] <- xmlToList(xmlParse(paste0("http://api.geonames.org/extendedFindNearby?lat=", # short (findNearby)
      #                                         md$Latitude[i], "&lng=", short (findNearby)
      #                                         md$Longitude[i], "&username=xavidp"))) short (findNearby)
      data.xml[[i]] <- xmlParse(paste0("http://api.geonames.org/extendedFindNearby?lat=", # long (extendedFindNearby)
                                       md$Latitude[i], "&lng=", # long (extendedFindNearby)
                                       md$Longitude[i], "&username=xavidp") # long (extendedFindNearby)
      ) # long (extendedFindNearby)
      data[[i]] <- xmlToList(data.xml[[i]]) # long (extendedFindNearby)
      
      # Recupera les dades d'interès, amb coordenades primer per garantir que tenim dataframe sencer
      # fins i tot en casos en que3 no obtenim res com a noms de lloc
      lloc[[i]] <- c(md$cid[i],
                     md$Latitude[i],
                     md$Longitude[i],
                     # data[[i]]$geoname$toponymName, # variable quan es cerca via findNearby
                     # data[[i]]$geoname$name) # variable quan es cerca via findNearby
                     data[[i]][length(data[[i]])-2]$geoname$toponymName, # Província - en català
                     data[[i]][length(data[[i]])-1]$geoname$toponymName, # Ciutat
                     data[[i]][length(data[[i]])  ]$geoname$toponymName  # Nom toponímic (lloc)
      )
      ...
      lloc[[i]] <- c(md$cid[i], NA, NA, "-", "-", "-")
    }
  }
  
  # convert list of results into data.frame
  lloc.df <- do.call(rbind.data.frame, lloc)
  colnames(lloc.df) <- c("Lat", "Lon", "Província", "Ciutat", "Nom_toponímic")
  md <- md %>% 
    mutate("Província"=lloc.df$Província,
           "Ciutat"=lloc.df$Ciutat,
           "Nom_toponímic"=lloc.df$Nom_toponímic
    )
