...
ct.map.table.all <- bscols(
  leaflet::leaflet(d) %>% 
    addTiles(tilesURL, 	attribution='Map tiles by <a href="http://server.arcgisonline.com">ArcGIS online</a>, <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a> &mdash; Map data &copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>') %>% 
    addMarkers(label = ~as.character(d$`nom vulgar (comestible)`),
               popup = ~as.character(d$Abundància)) %>%
    addLegend("bottomright", colors= "blue", labels="Some mushroom findings", title="In Catalonia"),
